...
def admin_add_channel_start(message):
    ...
    bot.reply_to(message, "➕ Yangi kanal username'ini yuboring (Masalan: `@kanal_nomi`):")
...
def admin_remove_channel_start(message):
    ...
    bot.reply_to(message, "🗑 O'chirmoqchi bo'lgan kanalingizni tanlang:", reply_markup=markup)
...
def callback_delete_channel(call):
    ...
    try:
        bot.edit_message_text(f"✅ <b>{channel_name}</b> o'chirildi!", call.message.chat.id, call.message.message_id, parse_mode="HTML")
    except:
        pass

@bot.message_handler(func=lambda message: message.text == "⚙️ Reklamani o'zgartirish" and message.from_user.id == ADMIN_ID)
def admin_change_ad_start(message):
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("📝 Reklama matnini o'zgartirish", callback_data="set_ad_text"))
    markup.add(types.InlineKeyboardButton("🔗 Reklama silka/username'ni o'zgartirish", callback_data="set_ad_user"))
    current_text = get_setting('ad_text')
    current_user = get_setting('ad_username')
    bot.reply_to(message, f"⚙️ **Hozirgi Reklama Sozlamalari:**\n\n<b>Username:</b> {current_user}\n\n<b>Matn:</b>\n{current_text}", parse_mode="HTML", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data in ["set_ad_text", "set_ad_user"])
def callback_set_ad(call):
    ...
    if call.data == "set_ad_text":
        ...
        bot.send_message(call.message.chat.id, "🔗 Yangi reklama username'ini yuboring (Masalan: `@reklamuchun1`):")

@bot.message_handler(func=lambda message: message.text == "🗑 Kino o'chirish" and message.from_user.id == ADMIN_ID)
...
def random_movie(message):
    ...
    if not movies:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.row(
            types.InlineKeyboardButton("📢 Reklama / Zakaz", url=f"https://t.me/{ad_user.replace('@', '')}"),
...
def reklama_menu(message):
    ad_user = get_setting('ad_username')
    bot.send_message(message.chat.id, f"📢 Reklama va xizmatlardan foydalanish uchun murojaat qiling: {ad_user}")
